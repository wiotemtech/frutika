...
$query = "SELECT * FROM custom_orders ORDER BY order_date DESC LIMIT 5";
$stmt = $pdo->query($query);
$orders = $stmt->fetchAll(PDO::FETCH_ASSOC);
// Update query to fetch the total orders
$query = "SELECT COUNT(*) as total_orders FROM custom_orders";
$stmt = $pdo->query($query);
$result = $stmt->fetch(PDO::FETCH_ASSOC);
$totalOrders = $result['total_orders'];
// Query to get the total number of unique customers
$query = "SELECT COUNT(DISTINCT customer_email) AS total_customers FROM custom_orders";
$stmt = $pdo->query($query);
$result = $stmt->fetch(PDO::FETCH_ASSOC);
$totalCustomers = $result['total_customers']; // This will hold the total number of unique customers
// Query to get the total sales
$query = "SELECT SUM(total) AS total_sales FROM custom_orders";
$stmt = $pdo->query($query);
$result = $stmt->fetch(PDO::FETCH_ASSOC);
...
$query = "
    ...
    FROM custom_orders 
    WHERE YEAR(order_date) = YEAR(CURDATE()) AND WEEK(order_date, 1) = WEEK(CURDATE(), 1)
";
$stmt = $pdo->query($query);
$result = $stmt->fetch(PDO::FETCH_ASSOC);
					...
					<!-- logo -->
						...
						<nav class="main-menu">
							<ul>
								...
								<li class="current-list-item"><a href="customorders.php">Custom Orders</a>
								</li>
								<li><a href="regularorders.php">Regular Orders</a></li></li>
